...
吸い出したものは、S-MU2000 が読む形に並べる。**並べるのは道具にやらせる。**

```
python tools/make_roms.py <吸い出したもの...> -o roms
```

zip でもディレクトリでもよい（混ぜてよい。ディレクトリは再帰的に見て中の zip も
開く）。名前は何でもよく、中身の SHA1 で見分けてバイトスワップも直す。
**EX（v2.01）が見つかればそれを優先する**（`tests/*.json` の指紋が EX で
焼いてあるため）。v1.01 を敢えて使うなら `--v1`。
